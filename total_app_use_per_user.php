...
<?php
$query =mysqli_query($dbconfig,"select EXTRACT(MONTH FROM start_time) as Month, EXTRACT(WEEK FROM start_time) "
        . "as week, (select distinct user_name) as unique_login, COUNT( application) as number_access, "
        . "SUM(TIME_TO_SEC(`Total_system_hours`))/60/60 as Total_system_hours from monitored_info group by "
        . "EXTRACT(MONTH FROM start_time), EXTRACT(WEEK FROM start_time),user_name")or die("this code is not running");
?>
    ...
    <tr>
        ...
        <th>TOTAL TIME OF APPLICATION USE (Min)</th>
    </tr>
    </thead>
    <tbody>
    <?php
            ...
            echo"no corrent month value";
             ...
             ?>
                                    ...
                                    <tr>
                                        ...
                                        <td> <?php echo $row2['unique_login'];?></td>
...
$sql="select EXTRACT(MONTH FROM start_time) as Month, EXTRACT(WEEK FROM start_time) "
        . "as week, user_name, (select distinct user_name) as unique_login, COUNT( application) as number_access, "
        . "SUM(TIME_TO_SEC(`Total_system_hours`))/60/60 as Total_system_hours from monitored_info group by "
        . "EXTRACT(MONTH FROM start_time), EXTRACT(WEEK FROM start_time),user_name";
