...
mysqli_select_db($dbconfig,"ci_db") or die("cannot select DB");
    ...
    <?php
    $query =mysqli_query($dbconfig,"SELECT EXTRACT(MONTH FROM start_time) as Month, count(DISTINCT user_name) "
            . "as unique_logins, COUNT( application) as number_access, application as application_name, "
            . "SUM(TIME_TO_SEC(`Total_system_hours`))/60/60 as Total_system_hours from monitored_info group by "
            . "EXTRACT(MONTH FROM start_time), application")or die("this code is not running");
    ?>
...
$sql="SELECT EXTRACT(MONTH FROM start_time) as Month,count(DISTINCT user_name) as unique_logins, COUNT( application) as number_access, "
                    . "application as application_name, SUM(TIME_TO_SEC(`Total_system_hours`))/60/60 as Total_system_hours "
                    . "from monitored_info group by EXTRACT(MONTH FROM start_time), application;";
